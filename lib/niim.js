...
const fs = Object.assign(require('fs-ext'), require('fs'));
const os = require('os');
 ...
 *
 * Snapshotting is done synchronously and with file locks, so many niim processes
 * together with large history files might cause performance issues. OS-level file
 * synchronization is managed with advisory file locks.
 *
...
exports.recordHistory = function nimm$recordHistory(repl) {
  ...
  if (repl)
    debuggerState.innerREPLHistory = [].concat(repl.history).slice(-maxLength);
  debuggerState.outerREPLHistory = [].concat(ni.repl.history).slice(-maxLength);

  if (!config.history && !process.env.NIIM_REPL_HISTORY)
    return;

  filename = (process.env.NIIM_REPL_HISTORY || config.history)
  ...
  if (!history) {
    ...
    history.outer = history.outer.concat(debuggerState.outerREPLHistory).slice(-maxLength);
    if (!history.inner[config.programName])
      history.inner[config.programName] = [];
    history.inner[config.programName] = history.inner[config.programName].concat(debuggerState.innerREPLHistory).slice(-maxLength);
  }
...
process.on('SIGINT', exports.recordHistory);
process.on('SIGQUIT', exports.recordHistory);
process.on('SIGHUP', exports.recordHistory);
process.on('exit', exports.recordHistory);
