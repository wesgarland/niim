...
function pxGet(target, prop) {
  if (target.patches.fn[prop] === true)
    target.patches.fn[prop] = new WrapperFactory(target.patches.label, prop);

  if (target.patches.get[prop])       /* patched getter? */
    return target.patches.get[prop];
  if (target.patches.fn[prop])        /* patched function? */
...
process.stdin = new Proxy(realStdin, {
  ...
  ownKeys: function niimPreload$$stdin$ownKeys(target) {
    let seen = {};
    return []
      .concat(Object.getOwnPropertyNames(realStdin))
      .concat(Object.getOwnPropertySymbols(realStdin))
