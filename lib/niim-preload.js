...
const stdinPatches  = { setter: {}, getter: {}, value: {}, fn: {}, label: 'stdin' };
const stdoutPatches = { setter: {}, getter: {}, value: {}, fn: {}, label: 'stdin' };
...
if (process.argv[1].indexOf("niim/niim") !== -1)
  debugger; /* XXX */
...
stdinPatches.value.isTTY    = { obj: debuggerInfo, prop: 'isTTY' };
stdoutPatches.value.rows    = { obj: debuggerInfo, prop: 'rows' };
stdoutPatches.value.columns = { obj: debuggerInfo, prop: 'columns' };
...
function pxGet(target, prop) {
  if (target.patches.fn[prop] === true) { /* patch is a function wrapper */
    target.patches.getter[prop] = () => new WrapperFactory(target.patches.label, prop);
    delete target.patches.fn[prop];
  }
